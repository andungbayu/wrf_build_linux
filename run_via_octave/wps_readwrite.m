...
% define io
inputfile='namelist.wps.default';
...
s_enddate=datestr(numenddate,'yyyy-mm-dd_HH:MM:SS');
...
while ischar(tline)
    ...
    disp(tline)
    if (t==4),
        fprintf(fo,'%s\n',[' start_date = ','''',s_startdate,''',''',s_startdate,''',''',s_startdate,''',']);
        fprintf(fo,'%s\n',[' end_date   = ','''',s_enddate,''',''',s_enddate,''',''',s_enddate,''',']);
    elseif (t>4)&&(t<=5)
        % skip writing 
    else 
        fprintf(fo,'%s\n',tline);
